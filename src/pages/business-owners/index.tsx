...
import { useRoqClient, useBusinessOwnerFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<BusinessOwnerInterface, unknown>;

interface BusinessOwnerListPageProps {
...
export function BusinessOwnerListPage(props: BusinessOwnerListPageProps) {
  const {
    filters = {},
    titleProps = {},
      ...
      convertQueryToPrismaUtil(
        {
          ...
          searchTermKeys: ['business_name.contains', 'business_address.contains', 'business_contact.contains'],
          ...params.filters,
        },
        'business_owner',
      ),
  ...
  const { data, error, isLoading, mutate } = useBusinessOwnerFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.business_owner.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: BusinessOwnerInterface) => {
    if (hasAccess('business_owner', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/business-owners/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'business_name', header: 'Business Name', accessorKey: 'business_name' },
    { id: 'business_address', header: 'Business Address', accessorKey: 'business_address' },
    { id: 'business_contact', header: 'Business Contact', accessorKey: 'business_contact' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/business-owners/view/${record.id}`} passHref legacyBehavior>
                <Button
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="business_owner" />
        </Flex>
