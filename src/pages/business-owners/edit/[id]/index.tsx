...
import { useRoqClient, useBusinessOwnerFindFirst } from 'lib/roq';
...
function BusinessOwnerEditPage() {
  ...
  const { data, error, isLoading, mutate } = useBusinessOwnerFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: BusinessOwnerInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.business_owner.update({
        data: values as RoqTypes.business_owner,
        where: {
          id,
        },
      });
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          ...
          <TextInput
            error={formik.errors.business_address}
          ...
          <TextInput
            error={formik.errors.business_contact}
