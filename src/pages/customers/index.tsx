...
import { useRoqClient, useCustomerFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<CustomerInterface, unknown>;
...
export function CustomerListPage(props: CustomerListPageProps) {
      ...
      convertQueryToPrismaUtil(
        {
          ...
          searchTermKeys: ['name.contains', 'address.contains', 'contact_number.contains'],
          ...params.filters,
        },
        'customer',
      ),
  ...
  const { data, error, isLoading, mutate } = useCustomerFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.customer.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: CustomerInterface) => {
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'contact_number', header: 'Contact Number', accessorKey: 'contact_number' },
