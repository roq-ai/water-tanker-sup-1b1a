...
import { useRoqClient, useSupplierFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<SupplierInterface, unknown>;
...
export function SupplierListPage(props: SupplierListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSupplierFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.supplier.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: SupplierInterface) => {
    if (hasAccess('supplier', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
