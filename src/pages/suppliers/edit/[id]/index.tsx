...
import { useRoqClient, useSupplierFindFirst } from 'lib/roq';
...
function SupplierEditPage() {
  ...
  const { data, error, isLoading, mutate } = useSupplierFindFirst(queryParams, {}, { disabled: !id });
  ...
  const handleSubmit = async (values: SupplierInterface, { resetForm }: FormikHelpers<any>) => {
    ...
    try {
      ...
      const updated = await roqClient.supplier.update({
        data: values as RoqTypes.supplier,
        where: {
          id,
        },
      });
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper wrapperProps={{ mb: 3 }}>
          <ImagePicker ref={imageUploaderRef} onChange={handleImageChange} entity="supplier" entityId={id} />
        </FormWrapper>
