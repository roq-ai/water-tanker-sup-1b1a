...
import { useOperationsManagerCount, useCustomerCount, useBusinessOwnerCount } from 'lib/roq';

import { OperationsManagerListPage as ShowCaseList } from 'pages/operations-managers';

function DashboardPage() {
  const { session } = useSession();

  const operationsManagerStat = useOperationsManagerCount();

  const customerStat = useCustomerCount();

  const businessOwnerStat = useBusinessOwnerCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Operations Managers',
      result: operationsManagerStat?.data,
      isLoading: operationsManagerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Customers',
      result: customerStat?.data,
      isLoading: customerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Business Owners',
      result: businessOwnerStat?.data,
      isLoading: businessOwnerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Operations Managers
                </Text>
                <NextLink href={`operations-managers`} passHref legacyBehavior>
                  <Button
